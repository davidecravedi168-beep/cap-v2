...
function rotatedModels(seed){
  const start=hash(seed)%BLOCKRUN_MODELS.length;
  return BLOCKRUN_MODELS.map((_,i)=>BLOCKRUN_MODELS[(start+i)%BLOCKRUN_MODELS.length]);
}
...
async function callProvider({name,url,model},system,user,ms){
  ...
  try{
    ...
    if(!r.ok){
      const detail=d?.error?.message||d?.error||`${name} ${r.status}`;
    ...
    if(e?.name==='AbortError')throw new Error(`${name} timeout`);
    throw e;
  }finally{
    clearTimeout(timer);
  }
...
async function resilient(system,user){
  ...
  const attempts=[
    ...
    {provider:'Vireonix',url:VIREONIX_URL,model:'auto',targetMs:8500},
    ...models.slice(1).map(model=>({provider:'BlockRun',url:BLOCKRUN_URL,model,targetMs:4500}))
  ];
  for(const a of attempts){
    ...
    try{
      return await callProvider({name:a.provider,url:a.url,model:a.model},system,user,timeout);
    }catch(e){
      failures.push({provider:a.provider,model:a.model,error:safeReason(a.provider,e)});
    }
  }
  const err=new Error('Capacità gratuita temporaneamente non disponibile. Riprova tra poco.');
...
async function execute(job){
  ...
  return{
    ...
    model:result.model,
    qualityReport:`Risposta completata tramite ${result.provider} (${result.model}). Modalità zero-euro; nessuna azione esterna eseguita.`,
    contributions:[{agent:'Direttore',provider:result.provider,model:result.model,text:result.text}],
    failures:[]
  };
}
export default{
  async fetch(req){
    const h=cors(req);
    if(!h)return json({error:'origin-not-allowed',zeroCost:true},403,{'content-type':'application/json'});
    ...
    if(req.method==='GET'&&u.pathname==='/health'){
      return json({
        ok:true,
        zeroCost:true,
        providers:['BlockRun','Vireonix'],
        engines:BLOCKRUN_MODELS.length+1,
        blockRunModels:BLOCKRUN_MODELS,
        strategy:'rotating-free-models-with-bounded-fallback',
    ...
    if(req.method==='POST'&&u.pathname==='/v1/jobs'){
      try{
        ...
        return json({status:'completed',zeroCost:true,id:job.id||null,...out},200,h);
      }catch(e){
        ...
        return json({
          ...
          error:'Capacità gratuita temporaneamente non disponibile. The Office ha già provato più motori gratuiti; riprova tra circa 15 secondi.',
          failures:Array.isArray(e?.failures)?e.failures:[]
        },503,h);
      }
    }
