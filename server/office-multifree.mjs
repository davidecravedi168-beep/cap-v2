...
export const MODEL_BOARD_VERSION='2026-09-15-v1';

const B=(model)=>({provider:'BlockRun',url:BLOCKRUN_URL,model,targetMs:5200});
const V=()=>({provider:'Vireonix',url:VIREONIX_URL,model:'auto',targetMs:6000});
...
export const ROLE_MODEL_BOARD={
  ...
  Lumen:{purpose:'ricerca sui materiali forniti e verifica',route:[B('nvidia/nemotron-3.5-lightning'),B('nvidia/nemotron-3-ultra-550b'),P(),V()]},
  Coda:{purpose:'coding, documenti e costruzione',route:[B('cohere/north-mini-code'),B('poolside/laguna-xs-2.1'),B('nvidia/nemotron-3-ultra-550b'),V()]},
  Mosaic:{purpose:'materiali e multimodale quando l’input lo consente',route:[B('nvidia/nemotron-3-nano-omni-30b-a3b-reasoning'),B('nvidia/llama-3.2-11b-vision'),B('nvidia/nemotron-3.5-lightning'),V()]},
  Sage:{purpose:'strategia e scenari',route:[B('nvidia/nemotron-3-ultra-550b'),B('nvidia/nemotron-3.5-lightning'),V(),P()]},
  Aegis:{purpose:'sicurezza, rischio e controlli',route:[B('nvidia/nemotron-3-nano-omni-30b-a3b-reasoning'),B('nvidia/nemotron-3-ultra-550b'),V()]},
  Verity:{purpose:'controprova indipendente',route:[V(),B('nvidia/nemotron-3.5-lightning'),B('nvidia/nemotron-3-nano-omni-30b-a3b-reasoning')]},
  Ledger:{purpose:'numeri e finanza',route:[B('nvidia/nemotron-3-ultra-550b'),B('nvidia/nemotron-3-nano-omni-30b-a3b-reasoning'),V()]},
  Archivist:{purpose:'memoria e sintesi del contesto fornito',route:[B('nvidia/nemotron-3.5-lightning'),B('nvidia/nemotron-3-ultra-550b'),V()]},
  Qualita:{purpose:'quality gate',route:[V(),B('nvidia/nemotron-3-nano-omni-30b-a3b-reasoning'),B('nvidia/nemotron-3.5-lightning')]},
};
...
function cors(req){
  ...
  return{'content-type':'application/json','access-control-allow-origin':o||ORIGIN,'access-control-allow-methods':'GET,POST,OPTIONS','access-control-allow-headers':'Content-Type','cache-control':'no-store'};
}
...
async function callAttempt(a,system,user,ms){
  if(a.pollinations){
    const d=await postJson(a.url,{model:'openai',messages:[{role:'system',content:system},{role:'user',content:user}],private:true,temperature:0.2},ms);
    const text=d?.choices?.[0]?.message?.content||d?.choices?.[0]?.text||'';
    if(!String(text).trim())throw new Error('Pollinations empty response');
    return{provider:'Pollinations',model:d?.model||a.model,text:String(text)};
  }
  const d=await postJson(a.url,{model:a.model,messages:[{role:'system',content:system},{role:'user',content:user}],max_tokens:1700,temperature:0.2},ms);
  const text=d?.choices?.[0]?.message?.content||d?.choices?.[0]?.text||'';
  if(!String(text).trim())throw new Error(`${a.provider} empty response`);
  return{provider:a.provider,model:d?.model||a.model,text:String(text)};
}
...
async function resilient(agent,user){
  ...
  const route=routeForAgent(agent).route;
  for(const a of route){
    const remaining=deadline-Date.now()-350;
    if(remaining<MIN_ATTEMPT_MS)break;
    const timeout=Math.max(MIN_ATTEMPT_MS,Math.min(a.targetMs||5000,remaining));
    try{return{...(await callAttempt(a,systemPrompt(agent),user,timeout)),agent,failures};}
    catch(e){failures.push({provider:a.provider,model:a.model,error:safeReason(a.provider,e)});}
  }
...
export default{async fetch(req){
  ...
  if(req.method==='GET'&&u.pathname==='/health')return json({ok:true,zeroCost:true,modelBoardVersion:MODEL_BOARD_VERSION,roleRouting:true,modelBoard:publicBoard(),providers:['BlockRun','Pollinations','Vireonix'],strategy:'role-aware-zero-cost-routing',hardDeadlineSeconds:HARD_DEADLINE_MS/1000},200,h);
